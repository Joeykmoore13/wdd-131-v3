...
function filterRecipes() {
	...
	let filteredRecipes = recipes.filter(function (recipe) {
		return (recipe.name.includes(query.toLowerCase()) ||
			recipe.description.includes(query.toLowerCase()) ||
			recipe.tags.find(tag => tag.toLowerCase().includes(query.toLowerCase())));
	})

	let recipes_section = document.querySelector(".recipes");
	recipes_section.innerHTML = getRecipesHTML(filteredRecipes);
}

function getRecipesHTML(recipes)
{
	let HTML = "";
	recipes.forEach(recipe => {HTML = HTML + getRecipeHTML(recipe)});
	return HTML;
}

function getRecipeHTML(recipe) {
	return `<section class="recipe">
                <img src="${recipe.image}" alt="An im age of a ${recipe.name}">

                <div>
					${getTagsHTML(recipe.tags)}
                    <p class="recipe-title">${recipe.name}</p>

                    <span class="rating" role="img" aria-label="Rating: ${recipe.rating} out of 5 stars">
                        ${getStarsHTML(Math.floor(recipe.rating))}
                    </span>
...
function getTagsHTML(tags) {
	let HTML = "";
	tags.forEach(tag => {
		HTML = HTML + `<p class="search-tag">${tag}</p>`
	});
	return HTML;
}
...
function getStarsHTML(rating) {
	...
	for (let i = 0; i < rating; i++) {
		HTML = HTML + `<span aria-hidden="true" class="icon-star">⭐</span>`
	}

	console.log(rating);

	for (let i = 5 - rating; i > 0; i--) {
		HTML = HTML + `<span aria-hidden="true" class="icon-star-empty">☆</span>`
	}
	return HTML;
}

//  JavaScript will be added to show a random recipe on the page.
// The user can search for keywords that will be compared to the recipe name, description, and tags to
// display recipes that contain the search keywords.Those recipes will be sorted by recipe name.

let recipes_section = document.querySelector(".recipes");
// recipes_section.innerHTML = getRecipesHTML(recipes);
recipes_section.innerHTML = getRecipeHTML(recipes[Math.floor(Math.random() * recipes.length)]);

document.querySelector("#search-bar img").addEventListener('click', filterRecipes);
